...
/** 서버가 어느 시간대에 있든 한국 시간으로 표기한다 (Vercel 함수는 UTC). */
function kstNow(date = new Date()) {
...
/** Airtable 레코드로 바로 가는 관리자 링크. 식별자가 없으면 빈 문자열. */
function adminUrl(recordId) {
...
function buildMessage(lead, recordId, at = new Date()) {
  const lines = [
    '<b>새 상담 신청</b>',
    '',
    `이름: ${esc(lead.name)}`,
  ...
  const url = adminUrl(recordId);
  if (url) lines.push('', `<a href="${esc(url)}">관리자 페이지에서 열기</a>`);
...
export const TELEGRAM_INTERNAL = { buildMessage, esc, kstNow, adminUrl, TIMEOUT_MS };
