...
export async function handleLead(body, ip) {
  ...
  try {
    ...
    // 레코드는 이미 저장됐다. 여기서 알림을 기다려도 접수 자체는 안전하다.
    // await하지 않으면 Vercel이 응답 직후 함수를 얼려 전송이 끊긴다(실측: 5초 타임아웃).
    // notifyLead는 예외를 던지지 않지만, 어떤 경우에도 응답이 바뀌지 않도록 한 겹 더 감싼다.
    await notifyLead(clean, id).catch(() => {});
